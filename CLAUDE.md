...
- **Personalisieren grid breakpoints (refactored 2026-05-01):** 2-column at ≥481px, 1-column at ≤480px. Card aspect-ratio `1/1` at ≥769 (overlay layout — same as product cards: image fills card, `__content` absolute at `bottom: 0`, title + desc stack with 12px gap, long desc pushes title up). At ≤768 cards revert to stacked layout (image-top + colour-block-below) because overlay typography can't shrink small enough for narrow phones. Title structurally lives inside `__content` (moved 2026-05-01); at ≤768 it's positioned `position: absolute; bottom: 100%; margin-bottom: 8px` so it overflows visually onto the image's bottom edge. Typography ≥481 uses card-width-proportional clamps `clamp(20px, calc(3.28vw - 1.97px), 40px)` (title, 6.56% of card) / `clamp(13px, calc(1.31vw - 0.787px), 16px)` (desc, 2.62% of card) — Figma 1280 reference. ≤480 has its own clamps 18-20 / 11-12. Above 1440 existing fluid block scales 40→53 / 16→21. See `project_card_scaling_system.md` for the canonical scaling system.
